document.addEventListener('DOMContentLoaded', () => {
  ...
  function fitSizes() {
    if (!gameArea) return;
    state.areaRect = gameArea.getBoundingClientRect();
  }
  ...
  function setBasketX(x) {
    ...
    const half = basketEl.offsetWidth / 2;
    const bounded = Math.min(Math.max(x - half, 8), w - basketEl.offsetWidth - 8);
    basketEl.style.left = bounded + 'px';
    state.basketX = bounded;
  }
          ...
          text: 'iya', action: () => {
            ...
            setTimeout(() => showModal({
              ...
              buttons: [{
                text: 'OK', action: () => {
                  closeModal();
                  window.location.href = "https://wa.me/62895604473976";
                }
              }]
            }), 120);
          }
